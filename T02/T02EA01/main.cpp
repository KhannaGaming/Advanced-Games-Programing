...
float g_clear_colour[4] = { 0.9f,0.5f,0.1f,1.0f };
bool isRedMax = false;
bool isGreenMax = false;
bool isBlueMax = false;
...
void RenderFrame(void)
{
	...
	else if (g_clear_colour[0] <= 0.0f)
	{
		isRedMax = false;
	}
	//Increase red value every frame
	if (isRedMax == false)
	{
		g_clear_colour[0] = g_clear_colour[0] + 0.00001f;
	}
	else
	{
		g_clear_colour[0] = g_clear_colour[0] - 0.00001f;
	...
	else if (g_clear_colour[1] <= 0.0f)
	{
		isGreenMax = false;
	}
	//Increase green value every frame
	if (isGreenMax == false)
	{
		g_clear_colour[1] = g_clear_colour[1] + 0.00001f;
	}
	else
	{
		g_clear_colour[1] = g_clear_colour[1] - 0.00001f;
	...
	//Increase blue value every frame
	if (isBlueMax == false)
	{
		g_clear_colour[2] = g_clear_colour[2] + 0.00001f;
	}
	else
	{
		g_clear_colour[2] = g_clear_colour[2] - 0.00001f;
