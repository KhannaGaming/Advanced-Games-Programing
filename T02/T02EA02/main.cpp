...
HRESULT InitialiseWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	//Create Window
	g_hInst = hInstance;
	RECT rc = { 0, 0, g_rect_width, g_rect_height };
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		//Left mouse decreasing red
	case WM_LBUTTONDOWN:
		g_clear_colour[0] = g_clear_colour[0] - 0.001f;		
		break;
	case WM_MOUSEMOVE:
		g_x = LOWORD(lParam);
		g_y = HIWORD(lParam);
		g_clear_colour[0] = g_x / g_rect_width;
		g_clear_colour[1] = g_y / g_rect_height;
		break;
	case WM_PAINT:
