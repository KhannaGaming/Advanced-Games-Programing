...
void DirectInput::CheckKeysPressed(SceneNode* g_cam_node, SceneNode* g_root_node)
{
	...
	if (g_keyboard_keys_state[DIK_W] & 0x80)
	{
		...
		g_root_node->update_collision_tree(&identity, 1.0f);
			...
			rootMovingNode.push_back(g_cam_node);
			SceneNode* nodeToCheck = g_cam_node->check_collision(g_root_node,rootMovingNode);
			...
			if (nodeToCheck != nullptr)
			{
				if (nodeToCheck->isMoveable())
				{
					...
					nodeToCheck->IncPos(dir.x*moveAmount, dir.y*moveAmount, dir.z*moveAmount, g_root_node, rootMovingNode);
