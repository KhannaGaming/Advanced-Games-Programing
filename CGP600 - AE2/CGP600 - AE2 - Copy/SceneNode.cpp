...
bool SceneNode::IncPos(float xAmount, float yAmount, float zAmount, SceneNode* root_node, vector<SceneNode*> nodesAlreadymoved)
{
	float old_x = m_x;
	float old_y = m_y;
	float old_z = m_z;
	...
	SceneNode* nodeToCheck = check_collision(root_node, nodesAlreadymoved);
	...
	if (nodeToCheck != nullptr)
	{
		if (nodeToCheck->isMoveable())
		{
			...
			nodesAlreadymoved.push_back(this);
			nodeToCheck->IncPos(dir.x*xAmount, dir.y*yAmount, dir.z*zAmount, root_node, nodesAlreadymoved);
		}
		else
		{
	//	m_x = old_x;
...
bool SceneNode::IncRotation(float xAmount, float yAmount, float zAmount, SceneNode* root_node, vector<SceneNode*> nodesAlreadymoved)
{
	...
	//check for collision of this node (and children) against all other nodes
	if (check_collision(root_node,nodesAlreadymoved) != nullptr)
	{
		nodesAlreadymoved.push_back(this);
...
bool SceneNode::IncScale(float scaleAmount, SceneNode* root_node, vector<SceneNode*> nodesAlreadymoved)
{
...
bool SceneNode::LookAt_XZ(float xWorld, float zWorld, SceneNode* root_node)
{
	...
	// so start at root node passing in identity matrix
	root_node->update_collision_tree(&identity, 1.0f);
	vector<SceneNode*> rootMovingNode;
	rootMovingNode.push_back(this);
	//check for collision of this node (and children) against all other nodes
	if (check_collision(root_node, rootMovingNode) != nullptr)
	{
		// if collision restore state
		m_yAngle = old_y;
		return true;
...
bool SceneNode::MoveForward(float distance, SceneNode* root_node)
{
	...
	vector<SceneNode*> rootMovingNode;
	rootMovingNode.push_back(this);
	SceneNode* nodeToCheck = check_collision(root_node, rootMovingNode);
	//check for collision of this node (and children) against all other nodes
	if (nodeToCheck != nullptr)
	{
...
bool SceneNode::MoveForwardIncY(float distance, SceneNode* root_node)
{
	...
	if (nodeToCheck != nullptr)
	{
		if (nodeToCheck->isMoveable())
		{
			...
			XMVECTOR dir = nodeToCheck->GetPos() - GetPos();
			nodeToCheck->IncPos(dir.x*distance, dir.y*distance, dir.z*distance, root_node, rootMovingNode);
			m_x = old_x;
			m_y = old_y;
			m_z = old_z;
		}
...
SceneNode* SceneNode::check_collision(SceneNode * compare_tree, vector<SceneNode*> nodesAlreadyMoved)
{
	return check_collision(compare_tree, this,nodesAlreadyMoved);
}

SceneNode* SceneNode::check_collision(SceneNode * compare_tree, SceneNode * object_tree_root, vector<SceneNode*> nodesAlreadyMoved)
{
	...
	for (int i = 0; i < nodesAlreadyMoved.size(); i++)
	{
		if (nodesAlreadyMoved[i] == compare_tree) return nullptr;
	}
		...
		for (int i = 0; i < compare_tree->m_children.size(); i++)
		{
			...
			if (check_collision(compare_tree->m_children[i], object_tree_root, nodesAlreadyMoved) != nullptr) return compare_tree->m_children[i];
		}
		...
		for (int i = 0; i < m_children.size(); i++)
		{
			...
			if (m_children[i]->check_collision(compare_tree, object_tree_root, nodesAlreadyMoved) != nullptr)	return m_children[i];
		}
